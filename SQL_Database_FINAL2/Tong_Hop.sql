...
-- =============================================
-- 2. TẠO TRIGGERS TỰ ĐỘNG & LOGIC KHO (ĐÃ SỬA LỖI LOGIC)
-- =============================================

-- [TRIGGER QUAN TRỌNG] Tự động tạo phiếu nhập khi tạo Sản phẩm mới
-- Đã sửa: Reset số lượng về 0 TRƯỚC khi tạo phiếu nhập để tránh cộng dồn sai.
CREATE TRIGGER trg_AutoCreateImport_OnNewProduct
ON Products
AFTER INSERT
AS
BEGIN
    ...
    BEGIN
        ...
        -- Lý do: Để lát nữa khi BƯỚC 2 chạy, Trigger nhập kho sẽ cộng số lượng vào.
        UPDATE Products 
        SET SoLuong = 0 
        FROM Products p
        ...
        -- Trigger 'trg_AfterImport_Insert' sẽ chạy sau lệnh này để cộng kho lên lại.
        -- Gán mặc định UserID = 1 (Admin) vì bảng Product không có UserID.
...
CREATE TRIGGER trg_AfterImport_Insert
ON ImportReceipts
AFTER INSERT
AS
BEGIN
    ...
    IF EXISTS (
        ...
        HAVING (SUM(p.SoLuong) + SUM(i.NewQty)) > 100
    )
    BEGIN
        RAISERROR(N'Lỗi: Danh mục đã đầy (Max 100).', 16, 1);
...
CREATE TRIGGER trg_AfterExport_Insert
ON ExportReceipts
AFTER INSERT
AS
BEGIN
...
-- User 1:
INSERT INTO Users (FullName, BirthDate, Email, MailboxPassword) VALUES 
(N'Nguyễn Anh Kiệt', '2006-01-01', 'admin@kho.vn', 'admin_pass_123');

-- User 2: 
INSERT INTO Users (FullName, BirthDate, Email, MailboxPassword) VALUES 
(N'Nguyễn Thành Danh', '2006-05-15', 'manager@kho.vn', 'manager_pass_456');

-- User 3:
INSERT INTO Users (FullName, BirthDate, Email, MailboxPassword) VALUES 
(N'Nguyễn Ngọc Duy', '2002-08-20', 'ketoan@kho.vn', 'ketoan_pass_789');
...
-- (Phần Insert dữ liệu mẫu dài của bạn giữ nguyên, mình rút gọn hiển thị nhưng logic vẫn chạy đủ nếu bạn paste hết)
-- Kho 1: Thực phẩm
...
-- LƯU Ý: Vì có Trigger, nên khi Insert ở đây, hệ thống sẽ:
-- 1. Insert Product (SL=1) -> Trigger Reset về 0 -> Tạo phiếu nhập (SL=1).
-- 2. Trigger Phiếu nhập chạy -> Update Product lên 1.
-- Kết quả cuối cùng: Tồn kho = 1.
INSERT INTO Products (ProductName, SoLuong, DonViTinh, HanSuDung, NgayNhapKho, CategoryID, SupplierID)
SELECT 
    t.BaseName + ' - ' + t.Variant, 
    1, -- Mặc định tồn 1 (Sẽ kích hoạt Trigger nhập kho)
    t.Unit,
...
-- 4. KIỂM TRA KẾT QUẢ
-- Số lượng phải là 1 (không phải 0, cũng không phải 2)
SELECT * FROM Products;

-- Phải có 300 dòng phiếu nhập tự động
SELECT * FROM ImportReceipts;
