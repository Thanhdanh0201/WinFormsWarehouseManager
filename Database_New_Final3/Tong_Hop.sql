...
BEGIN
    ...
    DECLARE @RandReceiver INT;
    DECLARE @RandUser INT;
    DECLARE @RandDate DATETIME;

    -- 1. Lấy ngẫu nhiên 1 sản phẩm đang có hàng (>0)
    SELECT TOP 1 @RandProdID = ProductID, @CurrentStock = SoLuong
    FROM Products
    WHERE SoLuong > 0
    ORDER BY NEWID();

    -- 2. Lấy ngẫu nhiên Nhân viên và Người nhận
    SELECT TOP 1 @RandUser = UserID FROM Users ORDER BY NEWID();
    SELECT TOP 1 @RandReceiver = ReceiverID FROM Receivers ORDER BY NEWID();

    -- 3. Tính số lượng xuất ngẫu nhiên (Từ 1 đến 10, nhưng không được vượt quá tồn kho)
    SET @RandQty = FLOOR(RAND() * 10) + 1;
    IF @RandQty > @CurrentStock SET @RandQty = @CurrentStock;

    -- 4. Tạo ngày xuất ngẫu nhiên trong 30 ngày gần đây
    SET @RandDate = DATEADD(DAY, -FLOOR(RAND() * 30), GETDATE());

    -- 5. Thực hiện Insert
    -- (Trigger trg_AfterExport_Insert sẽ tự động trừ kho và ghi Log)
    IF @CurrentStock > 0 
    BEGIN
        INSERT INTO ExportReceipts (ExportDate, UserID, ReceiverID, ProductID, Quantity)
        VALUES (@RandDate, @RandUser, @RandReceiver, @RandProdID, @RandQty);
    END
